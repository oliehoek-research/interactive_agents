'''
Script to test using Ray for distributed training. Implements an 
asynchronous version of PPO.

Right now just test distributed sampling with a uniform policy. We'll
start with centralized sampling
'''
import gym
...
class Learner:
    ...
    def train(self):
        '''Does one iteration of PPO'''

        # Wait for the buffer to fill with data for the current policy
        # NOTE: How do we transfer data from the actors to the learner?
        # RLLib uses a task hierarchy where the trainer launches sampling tasks and collects their responses
        # We want to do direct interprocess communication, pulling samples
        pass
