'''Test ONNX export and import of recurrent networks'''
...
if __name__ == "__main__":
    ...
    dummy_input = (
        torch.as_tensor(env.observation_space.sample()).reshape((1,1,-1)),
        model.get_h0()
    )  # NOTE: For unknown reasons, this must be a tuple, not a list

    input_names = ["obs", "hidden"]
    output_names = ["action"]
    ...
    # NOTE: We cannot actually run the model without the ONNX runtime

    # NOTE: ONNX just uses torchscript under the hood, may be better to use torchscript directly
