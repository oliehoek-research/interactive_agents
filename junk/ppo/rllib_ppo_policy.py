...
# NOTE: Specialized to support curiosity
...
from algorithms.curiosity.curiosity_dense import CuriosityDense  # NOTE, Why do we need separate implementations of these?
from algorithms.curiosity.curiosity_vision import CuriosityVision

logger = logging.getLogger(__name__)


class PPOLoss:  # NOTE: This is what really defines PPO - way too many parameters, had to have been a better way
    def __init__(self,
                 dist_class,
                 model,
                 value_targets,
        ...
        """
        if valid_mask is not None:  # NOTE: Seems to be used for jagged trajectories
...
def ppo_surrogate_loss(policy, model, dist_class, train_batch):  # NOTE: This just wraps around the PPOLoss class
...
def kl_and_loss_stats(policy, train_batch):  # NOTE: These are used to train 
    return {
...
def postprocess_ppo_gae(policy,
                        ...
                        episode=None):  # NOTE: Computes advantages on the actors for greater efficiency (only possible because PPO uses no replay buffer)
...
def make_model(policy, obs_space, action_space, config):  # NOTE: Need a more complete model construction pipeline
