...
from interactive_agents.sampling import FrozenPolicy  # NOTE: Generic class representing TorchScript Serialized Policies
...
# NOTE: Much of the machinery around the "gauntlet" is common to all 
# environments, better to move the environment-specific code to the
# `environments` directory, and have a generic gauntlet script.
if __name__ == '__main__':
    args = parse_args()

    # NOTE: Should load a dictionary of policies
    # Load pre-trained policies
    policies = load_policies(args.path, args.pid)

    # NOTE: Need to change this to the listener environment
    ...
    # NOTE: Actually run the gauntlet experiments
    ...
    # NOTE: Runs the same set of experiments again with injected noise
